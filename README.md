...
```
$ pip install -r requirements.txt open-project-linter
```
Once you have installed the package, you should be able to run it from the
command line (see [Example usage](#Example usage)).

This should work with Python 2.7 as well, although Python 2 is not officially
supported.
...
run:
```
$ python openlinter/openlinter.py -d path/to/repository/
```

For more help, run `$ python openlinter/openlinter.py --help`.
